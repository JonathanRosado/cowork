...
function completeTurn(state, turn = null, options = {}) {
  ...
  // Attach the accumulated tokenUsage from the dedicated thread/tokenUsage/updated
  // event onto the finalTurn so extractTokenUsage() finds it. Without this the
  // turn object has id/items/status but no token fields — codex app-server emits
  // tokens out-of-band from turn/completed.
  if (state.finalTurn && state.tokenUsage && !state.finalTurn.tokenUsage) {
    state.finalTurn.tokenUsage = state.tokenUsage;
  }
...
function applyTurnNotification(state, message) {
  switch (message.method) {
    case "thread/tokenUsage/updated":
      // Codex app-server emits token usage as a dedicated event that fires
      // after the final agent message but before turn/completed. We retain
      // the latest `tokenUsage.total` on the capture state so completeTurn()
      // can attach it to the finalTurn object — extractTokenUsage() then
      // surfaces it into the ledger heartbeat entry.
      if (message.params?.tokenUsage?.total) {
        state.tokenUsage = message.params.tokenUsage;
      }
      break;
    case "thread/started":
